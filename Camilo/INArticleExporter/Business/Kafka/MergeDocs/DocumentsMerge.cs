...
namespace Imprensa
{
    namespace Business
    {
        public class DocumentsMerge : IDocumentsMerge
        {          
            private Queue<XDocument> savedDocument = new Queue<XDocument>();           
            protected readonly IPublicationInfo publicationInfo;

            public DocumentsMerge(IPublicationInfo publicationInfo)
            {               
               
                this.publicationInfo = publicationInfo;
            }

            public virtual KafkaArticleDocument GetMergedArticle(List<KafkaArticleDocument> xDocs)
            {
                xDocs = (from a in xDocs
                         orderby a.PageArticle.Article.INMateriaSeq, a.PageArticle.Article.Name
                              select a).ToList();

                return this.MergeArticle(xDocs);
            }

            protected KafkaArticleDocument MergeArticle(List<KafkaArticleDocument> xDocs)
            {
                var pageArticles = xDocs;

                var currentDoc = new XDocument();
                KafkaArticleDocument firstARticle = null/* TODO Change to default(_) if this is not a reference type */;

                for (int index = 0; index <= pageArticles.Count - 1; index++)
                {
                    var currentArticle = pageArticles[index].PageArticle.Article;
                    currentDoc = pageArticles[index].KafkaXDocument;
                    ...
                    {
                        var firtsXdoc = firstARticle.KafkaXDocument;
                        var FirstText = firtsXdoc.Element("materia").Element("textoHTML");
                        var FirstTextEstruturado = firtsXdoc.Element("materia").Element("textoEstruturado");

                        var firstDataTexto = firtsXdoc.Element("materia").Element("estrutura").Element("dataTexto");

                        var firstIdentifica = firtsXdoc.Element("materia").Element("estrutura").Element("identificacao");

                        var firstTitulo = firtsXdoc.Element("materia").Element("estrutura").Element("titulo");

                        var firstSubtitulo = firtsXdoc.Element("materia").Element("estrutura").Element("subtitulo");

                        var firstEmenta = firtsXdoc.Element("materia").Element("estrutura").Element("ementa");

                        var assinaturaNodes = currentDoc.Element("materia").Element("estrutura").Elements("assinaturas");

                        currentDoc.Element("materia").Element("metadados").Element("destaque");
                        try
                        {
                            if ((assinaturaNodes.Descendants("assinante").Count() > 0))
                            {
                                var assinaturasNode = firtsXdoc.Element("materia").Element("estrutura");
                                foreach (var node in assinaturaNodes)
                                    assinaturasNode.Add(node);
                            }
                        }
                        catch
                        ...
                        var dataTexto = currentDoc.Element("materia").Element("estrutura").Element("dataTexto");
                        var identifica = currentDoc.Element("materia").Element("estrutura").Element("identificacao");
                        var titulo = currentDoc.Element("materia").Element("estrutura").Element("titulo");
                        var subtitulo = currentDoc.Element("materia").Element("estrutura").Element("subtitulo");
                        var ementa = currentDoc.Element("materia").Element("estrutura").Element("ementa");

                        if ((string.IsNullOrEmpty(firstDataTexto.Value)))
                            firstDataTexto.Value = dataTexto.Value;

                        if ((string.IsNullOrEmpty(firstIdentifica.Value)))
                            firstIdentifica.Value = identifica.Value;

                        if ((string.IsNullOrEmpty(firstTitulo.Value)))
                            firstTitulo.Value = titulo.Value;

                        if ((string.IsNullOrEmpty(firstSubtitulo.Value)))
                            firstSubtitulo.Value = subtitulo.Value;

                        if ((string.IsNullOrEmpty(firstEmenta.Value)))
                            firstEmenta.Value = ementa.Value;

                        if (publicationInfo.TitleName != "DO1A")
                        {
                            var textoHTMLNodes = currentDoc.Element("materia").Element("textoHTML").Nodes();

                            foreach (var node in textoHTMLNodes)
                            {
                                var cdata = FirstText.DescendantNodes().OfType<XCData>().ToList();
