...
namespace Imprensa
{
    namespace Business
    {
        public class MergeRelated : DocumentsMerge, IDocumentsMerge
        {

            public MergeRelated(IPublicationInfo publicationInfo) : base(publicationInfo)
            {
            }
            public override KafkaArticleDocument GetMergedArticle(List<KafkaArticleDocument> xDocs)
            {
                xDocs = (from a in xDocs
                         orderby a.PageArticle.Article.INChildPosition
                         select a).ToList();

                var chidlArticles = (from a in xDocs
                                     where a.PageArticle.Article.HasParent
                                     select a).ToList();

                var parentArticle = base.MergeArticle(xDocs);
                ...
                {
                    chidlArticles.Add(parentArticle);
                    //AddChilds(chidlArticles, ref parentArticle.KafkaXDocument); TODO: FIX
                }

                return parentArticle;
            }

            private void AddChildsOrc(List<KafkaArticleDocument> childsArticles, KafkaArticleDocument parentXdoc)
            {
                var relatedArticles = parentXdoc.KafkaXDocument.Element("materia").Element("metadados").Element("materiasRelacionadas");

                if (relatedArticles == null)
                    throw new Exception("materiasRelacionadas is missing");
                childsArticles = (from a in childsArticles
                                  orderby a.PageArticle.Article.INChildPosition
                                  select a).ToList();
                var canReplace = true;
                var metadados = parentXdoc.KafkaXDocument.Element("materia").Element("metadados");


                var index = 0;

                foreach (var item in childsArticles)
                {
                    index = index + 1;

                    var element = new XElement("materiasRelacionadas");

                    var editionModel = item.PageArticle.EditionModel;

                    var webTitle = item.PageArticle.EditionModel.webEditionNumber;

                    var idxml = string.Format("{0}+{1}+{2}", webTitle, editionModel.PublicationDate.ToString("yyyyMMdd"), item.PageArticle.Article.INMateriaId);

                    var idMateria = new XElement("idXML", idxml);
                    var ordem = new XElement("ordem", index);
            ...
            private void AddChilds(List<KafkaArticleDocument> childsArticles,  XDocument parentXdoc)
            {
                var relatedArticles = parentXdoc.Element("materia").Element("metadados").Element("materiasComposicaoTexto");
                ...
                childsArticles = (from a in childsArticles
                                  ...
                                  select a).ToList();
                var canReplace = true;
                var metadados = parentXdoc.Element("materia").Element("metadados");
                var index = 0;
                ...
                foreach (var item in childsArticles)
                {
                    ...
                    var element = new XElement("materiasComposicaoTexto");

                    var idMateria = new XElement("idMateria", item.PageArticle.Article.INMateriaId);
                    var idOficio = new XElement("idOficio", item.PageArticle.Article.INOficioId);
                    var ordem = new XElement("ordem", index);
