...
namespace Imprensa
{
    namespace Business
    {
        public class MergeArticles : IMergeArticles
        {
            private readonly IDocumentMergeFactory mergeFactory;

            public MergeArticles(IDocumentMergeFactory mergeFactory)
            {
                this.mergeFactory = mergeFactory;
            }

            public List<KafkaArticleDocument> GetResults(List<KafkaArticleDocument> kafkaArticleDocs)
            {
                MergeSplittedArticles(kafkaArticleDocs);
                MergeRelatedArticles(kafkaArticleDocs);
                return kafkaArticleDocs;
            }

            private void MergeRelatedArticles(List<KafkaArticleDocument> kafkaArticleDocs)
            {

                List<IGrouping<string, KafkaArticleDocument>> parents = (from item in kafkaArticleDocs
                                                                         where item.PageArticle.Article.HasParent
                                                                         group item by item.PageArticle.Article.INParentId into Group
                                                                         select Group).ToList();

                if ((parents.Count == 0))
                    return;

                foreach (IGrouping<string, KafkaArticleDocument> parent in parents)
                {
                    string inParentId = parent.Key;
                    KafkaArticleDocument parentDoc = kafkaArticleDocs.First(x => x.PageArticle.Article.INMateriaId == inParentId);
                    parentDoc.PageArticle.Article.INChildPosition = "0";
                    var relatedARticles = new List<KafkaArticleDocument>();

                    var childs = (from a in kafkaArticleDocs
                                  where a.PageArticle.Article.INParentId == inParentId

                                  select a).ToList();
                    relatedARticles.Add(parentDoc);
                    relatedARticles.AddRange(childs);
                    var mergeRelated = mergeFactory.GetDocumentMerge(MergeType.Related);
                    var mergedDoc = mergeRelated.GetMergedArticle(relatedARticles);
                    kafkaArticleDocs.RemoveAll((x) => x.PageArticle.Article.INMateriaId == inParentId);
                    kafkaArticleDocs.Add(mergedDoc);
                }
            }
            private void MergeSplittedArticles(List<KafkaArticleDocument> kafkaArticleDocs)
            {

                var splittedArticlesIds = (from a in kafkaArticleDocs
                                           group a by a.PageArticle.Article.INMateriaId into Group
                                           where Group.Count() > 1
                                           select Group).ToList();

                foreach (IGrouping<string, KafkaArticleDocument> item in splittedArticlesIds)
                {

                    List<KafkaArticleDocument> splittedArticles = (from a in kafkaArticleDocs
                                                                   where a.PageArticle.Article.INMateriaId == item.Key
                                                                   select a).ToList();
                    kafkaArticleDocs.RemoveAll(a => a.PageArticle.Article.INMateriaId == item.Key);


                    var documentsMerge = mergeFactory.GetDocumentMerge(MergeType.Splitted);

                    var kafkaARticleDoc = documentsMerge.GetMergedArticle(splittedArticles);

                    kafkaArticleDocs.Add(kafkaARticleDoc);
                }
            }
        }
    }
